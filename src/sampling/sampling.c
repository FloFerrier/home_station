...
void sampling_task(void *params) {
    ...
    status = sensor_selfTest(); // Wait the end of self-test, can be a little longer
    ...
    while (1) {
        ...
        if ((status == SENSOR_OK) && (sensor_data_number > 0)) {
            //console_send("[Sampling] Temperature %.2f degC\r\n", sensor_data[0].temperature_in_deg);
            //console_send("[Sampling] Relative humidity %.0f per100\r\n", sensor_data[0].humidity_in_per100);
        }
    }
}