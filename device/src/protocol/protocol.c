...
void protocol_serialize(protocol_s protocol, const uint32_t string_len_max,
                        char *string) {
    ...
    (void)snprintf(string_request, STRING_REQUEST_SIZE_MAX - 1,
                   "{\"code\":\"%s\",\"message\":\"%s\"}",
                   PROTOCOL_REQUEST_CODE[protocol.request.code],
    ...
    if (protocol.data_nb > 0) {
        for (uint8_t i = 0; i < protocol.data_nb; i++) {
            ...
            (void)snprintf(string_data, STRING_DATA_ITEM_SIZE_MAX - 1,
                           "{\"name\":\"%s\",\"value\":%.1f,\"unit\":\"%s\"}%c",
                           protocol.data[i].name, protocol.data[i].value,
                           protocol.data[i].unit, comma);
            (void)strncat(string_list_data, string_data,
                          STRING_DATA_LIST_SIZE_MAX - 1);
        }
    }
    ...
    if (protocol.metadata_nb > 0) {
        for (uint8_t i = 0; i < protocol.metadata_nb; i++) {
            ...
            (void)snprintf(string_metadata, STRING_METADATA_ITEM_SIZE_MAX - 1,
                           "{\"name\":\"%s\",\"value\":\"%s\"}%c",
                           protocol.metadata[i].name,
                           protocol.metadata[i].value, comma);
            (void)strncat(string_list_metadata, string_metadata,
                          STRING_METADATA_LIST_SIZE_MAX - 1);
        }
    }

    /* Fillin final buffer */
    (void)snprintf(string, string_len_max,
                   "{\"request\":%s,\"data\":[%s],\"metadata\":[%s]}\r\n",
