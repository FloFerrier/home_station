...
STATIC void command_unknown(char *response, const uint32_t response_len_max) {
    (void)snprintf(response, response_len_max, "Command unknown, tap \"help\" to display all available command.\r\n");
}

STATIC void command_help(char *response, const uint32_t response_len_max) {
    uint32_t response_size = snprintf(response, response_len_max, "All available command with description below :\r\n");
    response += response_size;
    for(uint32_t i= COMMAND_HELP+1; i < sizeof(command_list)/sizeof(*command_list);i++) {
        response_size = snprintf(response, response_len_max, "  %s\r\t\t\t%s\r\n", command_list[i].name, command_list[i].desc);
        response += response_size;
    }
}

STATIC void command_reboot(char *response, const uint32_t response_len_max) {
    (void)snprintf(response, response_len_max, "%s", command_list[COMMAND_REBOOT].desc);
}
