...
namespace Antmicro.Renode.Peripherals.I2C {
    public class BME680 : II2CPeripheral, IProvidesRegisterCollection<ByteRegisterCollection> {
        ...
        public byte[] Read(int count = 0) {
            ...
            this.Log(LogLevel.Warning, "Read {0}", buf.Select(x => x.ToString("X")).Aggregate((x, y) => x + " " + y));
        ...
        private void DefineRegisters() {
            Registers.HumLsb.Define(this, 0x0).WithValueField(0, 8, out humLsb, FieldMode.Read);
            Registers.HumMsb.Define(this, 0x80).WithValueField(0, 8, out humMsb, FieldMode.Read);
            Registers.TempXlsb.Define(this, 0x0).WithValueField(0, 8, out tempXlsb, FieldMode.Read);
            Registers.TempLsb.Define(this, 0x0).WithValueField(0, 8, out tempLsb, FieldMode.Read);
            Registers.TempMsb.Define(this, 0x80).WithValueField(0, 8, out tempMsb, FieldMode.Read);
            Registers.PressXlsb.Define(this, 0x0).WithValueField(0, 8, out pressXlsb, FieldMode.Read);
            Registers.PressLsb.Define(this, 0x0).WithValueField(0, 8, out pressLsb, FieldMode.Read);
            Registers.PressMsb.Define(this, 0x80).WithValueField(0, 8, out pressMsb, FieldMode.Read);
            Registers.Config.Define(this, 0x0).WithValueField(0, 8, name: "Config");
            Registers.CtrlMeas.Define(this, 0x0).WithValueField(0, 8, name: "CtrlMeas");
            Registers.Status.Define(this, 0x0).WithValueField(0, 8, name: "Status");
            Registers.CtrlHum.Define(this, 0x0).WithValueField(0, 8, name: "CtrlHum");
            Registers.Reset.Define(this, 0x0).WithValueField(0, 8)
            ...
            Registers.Id.Define(this, 0x61).WithValueField(0, 8, FieldMode.Read, valueProviderCallback: _ => 0x61);
